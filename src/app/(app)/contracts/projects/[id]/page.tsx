...
import { InlineContractDetail } from "@/components/domain/InlineContractDetail";
...
export default async function ProjectPage({
  ...
  searchParams: Promise<{ status?: string; risk?: string; q?: string; doc?: string }>;
}) {
  const { id }                   = await params;
  const { status, risk, q, doc } = await searchParams;
    ...
    <div className="-mx-4 sm:-mx-6 -mt-4 sm:-mt-6 flex gap-0">
      ...
      <div className="flex-1 min-w-0">
        {doc ? (
          ...
          <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-10">
            <InlineContractDetail
              contractId={doc}
              workspace={workspace}
              projectId={id}
            />
          ...
          /* ── Project overview ── */
          <div className="px-4 sm:px-6 pt-4 sm:pt-6 pb-10 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {contracts.map((c) => (
                  <ContractCard
                    key={c.id}
                    contract={c}
                    href={`/contracts/projects/${id}?doc=${c.id}`}
                  />
                ))}
              </div>
            ) : (
